...
using namespace std;
using u32 = uint32_t;
struct Block { string tmp; size_t count; };

void worker_sort(const string &infile, size_t offset_items, size_t cnt, const string &tmp){
    FILE* f = fopen(infile.c_str(),"rb");
    if(!f){ perror("fopen"); exit(1); }
    fseek(f, offset_items * sizeof(u32), SEEK_SET);
    vector<u32> buf(cnt);
...
int main(int argc,char**argv){
    if(argc<4){ cerr<<"Usage: "<<argv[0]<<" <input> <threads> <total_items>\n"; return 1; }
    ...
    for(int i=0;i<threads;++i){
        ...
        string tmp = infile + ".blk." + to_string(i) + ".bin";
        blocks.push_back({tmp, cnt});
        ths.emplace_back(worker_sort, infile, offset, cnt, tmp);
        offset += cnt;
    }
    for(auto &t: ths) t.join();

    // k-way merge and write runs to outfile
    string outname = infile + ".thread.result.txt";
    FILE* out = fopen(outname.c_str(),"w");
    // readers
    struct R{ FILE* f; u32 cur; bool has; };
    vector<R> readers(blocks.size());
    for(size_t i=0;i<blocks.size();++i){
        readers[i].f = fopen(blocks[i].tmp.c_str(),"rb");
        readers[i].has = false;
        if(readers[i].f){
